...
# `initializeprobmap`), and their symbol lists overlap heavily, so the same observed-function
# batches get compiled repeatedly. Memoize on `srcsys` keyed by the (content-hashed) batch.
# Keyed by batch only: within one problem build the eval settings passed via `kws` are constant
# and `srcsys` (the init system) is built fresh per problem. Never auto-invalidated.
